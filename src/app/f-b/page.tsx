import HouseCard from '@/components/houses/HouseCard';
import { getFbs } from '@/services/f-b';

export const revalidate = 3600; // revalidate every hour

async function getFbProjects() {
  const fbs = await getFbs();
  return fbs.map(fb => ({
    title: fb.title,
    subtitle: 'F&B',
    imageUrl: fb.images[0],
    link: `/f-b/${fb.slug}`
  }));
}

export default async function Fb() {
  const fbProjects = await getFbProjects();

  if (!fbProjects || fbProjects.length === 0) {
    return (
      <div className="flex items-center justify-center h-screen">
        <p className="text-xl text-white">Trang vẫn đang cập nhật</p>
      </div>
    );
  }
  
  return (
    <div className="page-wrapper grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
      {fbProjects.map((project, index) => (
        <HouseCard
          key={index}
          title={project.title}
          subtitle={project.subtitle}
          imageUrl={project.imageUrl}
          link={project.link}
        />
      ))}
    </div>
  );
}
